...
RSpec.describe Qywx::Client::GroupRobotClient do
  include Qywx::Client::GroupRobotClient
  ...
  context 'NotifyFeedcard' do
    ...
    it 'notify success' do
      ...
      link = { title: 'title', url: 'message_url', picurl: 'pic_url' }
      result = notify_feedcard(:test, link)
      expect(result).to be_success
    end
  end
  context 'NotifyText' do
    it 'notify success no @' do
      body = '{"errmsg":"ok","errcode":0}'
      response = double(parsed_response: JSON.parse(body),
                        code: 200,
                        message: 'OK',
    ...
    it 'notify success @all by users' do
      body = '{"errmsg":"ok","errcode":0}'
      response = double(parsed_response: JSON.parse(body),
                        code: 200,
                        message: 'OK',
                        body: body)
      allow(subject).to receive(:post).and_return(response)
      result = notify_text(:test, 'message', at_users: ['@all'])
      expect(result).to be_success
    end
  end
